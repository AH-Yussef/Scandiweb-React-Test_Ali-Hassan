...
export default class ProductDetails extends Component {
  ...
  render() {
    ...
    const { primaryImg, inCart } = this.state;
      ...
      <div className='product-details container flex'>
        <div className='secondary-img-col flex'>
          {gallery.map((imgUrl, index) => (
            <div key={index} className={this.setSecondaryImgContainerClasses(index)} onClick={() => this.changePrimaryImg(index)}>
        ...
        <div className='info-container flex text-black'>
          ...
          <div className='desc fs-16 fw-regular' dangerouslySetInnerHTML={{ __html: description }}/>
        </div>
      </div>
    )
  }
  ...
  doesExistInCart = () => {
    ...
    if (cart.filter(item => item.id === this.props.productId).length > 0) return true;
    return false;
  }

  setAddToCartBtnClasses = () => {
    const { inStock } = this.state.product;
    let classes = 'add-to-cart-btn bg-accent text-white fs-16 fw-semi-bold upper-case vh-center select-none';
    if (this.doesExistInCart() || !inStock) classes += ' add-to-cart-disabled';
    return classes;
  }
}